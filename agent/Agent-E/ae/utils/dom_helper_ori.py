...
def wait_for_non_loading_dom_state(page: Page, max_wait_millis: int):
    max_wait_seconds = max_wait_millis / 1000
    end_time = asyncio.get_event_loop().time() + max_wait_seconds
    while asyncio.get_event_loop().time() < end_time:
        dom_state = page.evaluate("document.readyState")
...
def get_element_outer_html(element: ElementHandle, page: Page, element_tag_name: str|None = None) -> str:
    """
    ...
    """
    tag_name: str = element_tag_name if element_tag_name else page.evaluate("element => element.tagName.toLowerCase()", element)
