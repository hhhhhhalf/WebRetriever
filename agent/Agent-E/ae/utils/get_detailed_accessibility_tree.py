...
def __inject_attributes(page: Page):
    """
...
def __fetch_dom_info(page: Page, accessibility_tree: dict[str, Any], only_input_fields: bool):
    """
    ...
    def process_node(node: dict[str, Any]):
        ...
        if mmid:
            ...
            element_attributes = page.evaluate(js_code,
...
def get_node_dom_element(page: Page, mmid: str):
...
def get_element_attributes(page: Page, mmid: str, attributes: list[str]):
    return page.evaluate("""
        (inputParams) => {
...
def get_dom_with_accessibility_info() -> Annotated[dict[str, Any] | None, "A minified representation of the HTML DOM for the current webpage"]:
    """
...
def do_get_accessibility_info(page: Page, only_input_fields: bool = False):
    """
    ...
    # Playwright 1.47+ 移除了 page.accessibility，改用 CDP 协议获取
    try:
        accessibility_tree = page.accessibility.snapshot(interesting_only=True)  # type: ignore
    except AttributeError:
        cdp = page.context.new_cdp_session(page)
        cdp_tree = cdp.send("Accessibility.getFullAXTree")
        # 将 CDP 格式转换为 Playwright snapshot 兼容格式
        def _cdp_to_snapshot(nodes):
            """将 CDP AXTree 转为 Playwright accessibility.snapshot 兼容的嵌套 dict"""
            node_map = {}
            for n in nodes:
                node_id = n["nodeId"]
                ...
                if n.get("value", {}).get("value"):
                    entry["value"] = n["value"]["value"]
            ...
            for n in nodes:
                ...
                if parent_id and parent_id in node_map:
                    ...
                    if "children" not in parent:
                        parent["children"] = []
                    parent["children"].append(node_map[n["nodeId"]])
            # 根节点
            root_id = nodes[0]["nodeId"] if nodes else None
            return node_map.get(root_id, {"role": "WebArea", "name": ""})
        accessibility_tree = _cdp_to_snapshot(cdp_tree.get("nodes", []))
        cdp.detach()
