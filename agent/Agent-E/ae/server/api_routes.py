...
def process_command(command: str, playwright_manager: browserManager.PlaywrightManager, planner_max_chat_round: int, browser_nav_max_chat_round: int, llm_config:dict[str,Any]|None = None):
    """
    ...
    Args:
        ...
        playwright_manager (PlaywrightManager): The manager handling browser interactions and notifications.
    """
    playwright_manager.go_to_homepage() # Go to the homepage before processing the command
    current_url = playwright_manager.get_current_url()
    playwright_manager.notify_user("Processing command", MessageType.INFO)
    ...
    ag = AutogenWrapper.create(planner_agent_config, browser_nav_agent_config, planner_max_chat_round=planner_max_chat_round,
                                     browser_nav_max_chat_round=browser_nav_max_chat_round)
    command_exec_result = ag.process_command(command, current_url)  # type: ignore
    ...
    if is_terminating_message(command_exec_result.summary):
        playwright_manager.notify_user("DONE", MessageType.DONE)
    else:
        playwright_manager.notify_user("Max turns reached", MessageType.MAX_TURNS_REACHED)
