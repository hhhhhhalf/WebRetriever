...
def click(selector: Annotated[str, "The properly formed query selector string to identify the element for the click action (e.g. [mmid='114']). When \"mmid\" attribute is present, use it for the query selector."],
    ...
    browser_manager.highlight_element(selector, True)
    ...
    result = do_click(page, selector, wait_before_execution)
    # asyncio.sleep(0.1) # sleep for 100ms to allow the mutation observer to detect changes
    time.sleep(0.1) # sleep for 100ms to allow the mutation observer to detect changes
    unsubscribe(detect_dom_changes)
    browser_manager.take_screenshots(f"{function_name}_end", page)
    browser_manager.notify_user(result["summary_message"], message_type=MessageType.ACTION)
...
def do_click(page: Page, selector: str, wait_before_execution: float) -> dict[str, str]:
    """
    ...
    if wait_before_execution > 0:
        # asyncio.sleep(wait_before_execution)
    ...
    try:
        ...
        # element = asyncio.wait_for(
        #     page.wait_for_selector(selector, state="attached", timeout=2000),
        #     timeout=2000
        # )
        element = page.wait_for_selector(selector, state="attached", timeout=2000)
        if element is None:
        ...
        if element_tag_name == "option":
            element_value = element.get_attribute("value") # get the text that is in the value of the option
            parent_element = element.evaluate_handle("element => element.parentNode")
            # parent_element.evaluate(f"element => element.select_option(value=\"{element_value}\")")
        ...
        #perform_playwright_click(element, selector)
        msg = perform_javascript_click(page, selector)
...
def perform_playwright_click(element: ElementHandle, selector: str):
    """
    ...
    element.click(force=False, timeout=200)


def perform_javascript_click(page: Page, selector: str):
    """
    ...
    try:
        ...
        result:str = page.evaluate(js_code, selector)
