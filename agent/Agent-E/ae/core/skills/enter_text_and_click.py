...
def enter_text_and_click(
    ...
    Example usage:
    ```
    enter_text_and_click("[mmid='1234']", "Hello, World!", "[mmid='5678']", wait_before_click_execution=1.5)
    ```
    """
    ...
    browser_manager.highlight_element(text_selector, True)

    function_name = inspect.currentframe().f_code.co_name # type: ignore
    browser_manager.take_screenshots(f"{function_name}_start", page)

    text_entry_result = do_entertext(page, text_selector, text_to_enter, use_keyboard_fill=True)

    #browser_manager.notify_user(text_entry_result["summary_message"])
    ...
    if text_selector == click_selector:
        do_press_key_combination_result = do_press_key_combination(browser_manager, page, "Enter")
        if do_press_key_combination_result:
            ...
            browser_manager.notify_user(f"Pressed the Enter key successfully on element: \"{click_selector}\".", message_type=MessageType.ACTION)
        else:
            ...
            browser_manager.notify_user("Failed to press the Enter key on element \"{click_selector}\".", message_type=MessageType.ACTION)
    else:
        browser_manager.highlight_element(click_selector, True)

        do_click_result = do_click(page, click_selector, wait_before_click_execution)
        result["detailed_message"] += f' {do_click_result["detailed_message"]}'
        #browser_manager.notify_user(do_click_result["summary_message"])
