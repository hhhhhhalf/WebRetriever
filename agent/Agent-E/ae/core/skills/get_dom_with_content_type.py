...
def get_dom_with_content_type(
    ...
    extracted_data = None
    wait_for_non_loading_dom_state(page, 2000) # wait for the DOM to be ready, non loading means external resources do not need to be loaded
    user_success_message = ""
    if content_type == 'all_fields':
        user_success_message = "Fetched all the fields in the DOM"
        extracted_data = do_get_accessibility_info(page, only_input_fields=False)
    ...
    browser_manager.notify_user(user_success_message, message_type=MessageType.ACTION)
