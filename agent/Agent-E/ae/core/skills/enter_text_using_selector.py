...
def custom_fill_element(page: Page, selector: str, text_to_enter: str):
    """
    ...
    Example:
        custom_fill_element(page, '#username', 'test_user')
...
def entertext(entry: Annotated[EnterTextEntry, "An object containing 'query_selector' (DOM selector query using mmid attribute e.g. [mmid='114']) and 'text' (text to enter on the element)."]) -> Annotated[str, "Explanation of the outcome of this operation."]:
    """
    ...
    browser_manager.highlight_element(query_selector, True)
...
def do_entertext(page: Page, selector: str, text_to_enter: str, use_keyboard_fill: bool=True):
    """
    ...
    try:
        ...
        if use_keyboard_fill:
            ...
            # asyncio.sleep(0.1)
            time.sleep(0.1)
            press_key_combination("Control+A")
            # asyncio.sleep(0.1)
            time.sleep(0.1)
            press_key_combination("Backspace")
            # asyncio.sleep(0.1)
            time.sleep(0.1)
...
def bulk_enter_text(
    ...
    Example:
        ...
        results = bulk_enter_text(entries)
    ...
    for entry in entries:
        ...
        result = entertext(EnterTextEntry(query_selector=query_selector, text=text_to_enter))
