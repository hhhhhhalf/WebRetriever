...
def get_user_input(questions: Annotated[List[str], "List of questions to ask the user each one represented as a string"] ) -> dict[str, str]:  # noqa: UP006
    """
    ...
    if browser_manager.ui_manager:
        for question in questions:
            answers[question] = browser_manager.prompt_user(f"Question: {question}")
    else:
        answers = answer_questions_over_cli(questions)
    return answers
