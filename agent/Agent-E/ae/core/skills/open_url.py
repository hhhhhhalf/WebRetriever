...
def openurl(url: Annotated[str, "The URL to navigate to. Value must include the protocol (http:// or https://)."],
    ...
    browser_manager.get_browser_context()
    ...
    try:
        ...
        page.goto(url, timeout=timeout*1000) # type: ignore
    ...
    browser_manager.notify_user(f"Opened URL: {url}", message_type=MessageType.ACTION)
        # Get the page title
    title = page.title()
    url=page.url
