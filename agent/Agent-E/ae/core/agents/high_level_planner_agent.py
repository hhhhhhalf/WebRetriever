...
class PlannerAgent:
    def __init__(self, model_config_list, llm_config_params: dict[str, Any], system_prompt: str|None, user_proxy_agent:ConversableAgent,
                use_keypoints: False): # type: ignore
        """
        ...
        if use_keypoints:
            print("使用关键路径")
            # exit()
            system_message = LLM_PROMPTS["PLANNER_AGENT_PROMPT_KEYPOINTS"]
        else:
