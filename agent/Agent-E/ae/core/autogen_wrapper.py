...
class AutogenWrapper:
    ...
    @classmethod
    def create(cls, planner_agent_config: dict[str, Any], browser_nav_agent_config: dict[str, Any], agents_needed: list[str] | None = None,
                     save_chat_logs_to_files: bool = True, planner_max_chat_round: int = 50, browser_nav_max_chat_round: int = 10, use_keypoints: bool = False):
        """
        ...
        self.agents_map = self.__initialize_agents(agents_needed)
        ...
        def reflection_message(recipient, messages, sender, config): # type: ignore
            ...
            if next_step is None:
                ...
                if self.use_keypoints:
                    next_step += f"Keypoints: {self.keypoints}."
    ...
    def __initialize_agents(self, agents_needed: list[str]):
        """
        ...
        user_delegate_agent = self.__create_user_delegate_agent()
    ...
    def __create_user_delegate_agent(self) -> autogen.ConversableAgent:
        """
    ...
    def __create_planner_agent(self, assistant_agent: autogen.ConversableAgent):
        ...
        planner_agent = PlannerAgent(self.planner_agent_model_config, self.planner_agent_config["llm_config_params"], # type: ignore
                                     self.planner_agent_config["other_settings"].get("system_prompt", None), assistant_agent,
                                     use_keypoints=self.use_keypoints) # type: ignore
        return planner_agent.agent

    def process_command(self, command: str, current_url: str | None = None) -> autogen.ChatResult | None:
        """
        ...
        if self.use_keypoints:
            prompt = Template(LLM_PROMPTS["COMMAND_EXECUTION_PROMPT_KEYPOINTS"]).substitute(
                command=command, 
        ...
        try:
            ...
            print("开始测试")
            # result=self.agents_map["user"].a_initiate_chat( # type: ignore
            result=self.agents_map["user"].initiate_chat( # type: ignore
            ...
            # reset usage summary for al-l agents after each command
