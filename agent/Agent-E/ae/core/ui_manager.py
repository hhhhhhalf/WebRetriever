...
class UIManager:
    ...
    def handle_navigation(self, frame: Frame):
        """
        ...
        try:
            ...
            if self.overlay_is_collapsed:
                frame.evaluate(f"showCollapsedOverlay('{self.overlay_processing_state}', {js_bool});")
            else:
                frame.evaluate(f"showExpandedOverlay('{self.overlay_processing_state}', {js_bool});")

            #update chat history in the overlay
            self.update_overlay_chat_history(frame)
    ...
    def show_overlay(self, page: Page):
        """
        ...
        if not self.overlay_is_collapsed:
            ...
            return
        page.evaluate("showExpandedOverlay();")
    ...
    def update_overlay_show_details(self, show_details: bool, page: Page):
        """
        ...
        self.update_overlay_chat_history(page)


    def update_processing_state(self, state: str, page: Page):
        """
        ...
        try:
            ...
            page.evaluate(f"updateOverlayState('{self.overlay_processing_state}', {js_bool});")
        except Exception as e:
            logger.debug(f"JavaScript error: {e}")

    def update_overlay_chat_history(self, frame_or_page: Frame | Page):
        """
        ...
        try:
            frame_or_page.evaluate("clearOverlayMessages();")
            for message in self.conversation_history:
                ...
                if message["from"] == "user":
                    frame_or_page.evaluate(f"addUserMessage({safe_message});")
                else:
                   #choose chich message types to be shown depending on UI setting
                    ...
                    frame_or_page.evaluate(js_code)
    ...
    def command_completed(self, page: Page, command: str, elapsed_time: float|None = None):
        """
        ...
        """
        if not self.overlay_is_collapsed:
            page.evaluate("focusOnOverlayInput();")
            page.evaluate("commandExecutionCompleted();")
