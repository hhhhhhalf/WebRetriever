...
class PlaywrightManager:
    ...
    def __new__(cls, *args, **kwargs): # type: ignore
        ...
        if not hasattr(cls._thread_local, 'instance'):
            ...
            logger.debug(f"Playwright instance created for thread {threading.current_thread().name}")
    ...
    def __init__(self, cdp_url: str = "", headless: bool = False, gui_input_mode: bool = False, screenshots_dir: str = "", take_screenshots: bool = False):
        ...
        # 每个线程独立的 Playwright 和 browser_context
    ...
    def start_playwright(self):
        ...
        if not self._playwright:  # ✅ 改为实例变量
            self._playwright: Playwright = sync_playwright().start()
            logger.debug(f"Playwright started for thread {threading.current_thread().name}")  # ✅ 新增日志
    ...
    def stop_playwright(self):
        ...
        print(f"关闭浏览器 - 线程 {threading.current_thread().name}")  # ✅ 修改日志
        
        if self._browser_context is not None:  # ✅ 改为实例变量
        ...
        if self._playwright is not None:  # ✅ 改为实例变量
            self._playwright.stop()
            self._playwright = None
            logger.debug(f"Playwright stopped for thread {threading.current_thread().name}")  # ✅ 新增日志
    ...
    def create_browser_context(self):
        ...
        try:
            ...
            # 4️⃣ 设置默认超时
            self._browser_context.set_default_timeout(60000)
            self._browser_context.set_default_navigation_timeout(60000)
    ...
    def get_current_page(self) -> Page :
        ...
        try:
            ...
            logger.debug(f"Current page: {current_page.url if current_page else None} - 线程 {threading.current_thread().name}")  # ✅ 修改日志
            ...
            if current_page is not None:
                ...
                logger.warn(f"Browser context was closed. Creating a new one - 线程 {threading.current_thread().name}")  # ✅ 修改日志
                self._browser_context = None
                self._browser = None
                _browser_context:BrowserContext = self.get_browser_context() # type: ignore
    ...
    def go_to_homepage(self):
        page:Page = self.get_current_page(self)
