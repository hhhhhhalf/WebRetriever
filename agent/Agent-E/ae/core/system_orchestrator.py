...
class SystemOrchestrator:
    ...
    def initialize(self):
        ...
        self.autogen_wrapper = AutogenWrapper.create(self.planner_agent_config, self.browser_nav_agent_config, agents_needed=self.agent_names,
        ...
        self.browser_manager.async_initialize()

        if self.input_mode == "GUI_ONLY":
            browser_context = self.browser_manager.get_browser_context()
            browser_context.expose_function('process_task', self.receive_command) # type: ignore
    ...
    def start(self):
        ...
        if self.input_mode != "GUI_ONLY":
            self.command_prompt_loop()

        self.wait_for_exit()

    def command_prompt_loop(self):
        """
        Continuously reads and processes commands from the command prompt until an 'exit' command is received.
        """
        while not self.is_running:
            command: str = async_input("Enter your command (or type 'exit' to quit): ") # type: ignore
            self.process_command(command) # type: ignore
    ...
    def __orchestrate_command(self, command: str):
        ...
        if "user_notification" in orch_response:
            self.browser_manager.notify_user(orch_response["user_notification"]) # type: ignore
    ...
    def process_command(self, command: str):
        ...
        if command:
            ...
            current_url = self.browser_manager.get_current_url() if self.browser_manager else None
            ...
            if self.autogen_wrapper:
                self.browser_manager.update_processing_state("processing") # type: ignore
                orchestrated_command = self.__orchestrate_command(command)
                if orchestrated_command is not None:
                    result = self.autogen_wrapper.process_command(orchestrated_command, current_url)
                else:
                    result = self.autogen_wrapper.process_command(command, current_url)

                self.browser_manager.update_processing_state("done") # type: ignore
            end_time = time.time()
            elapsed_time = round(end_time - start_time, 2)
            logger.info(f"Command \"{command}\" took: {elapsed_time} seconds.")
            self.save_planner_chat_messages()
            if result is not None:
                ...
                if last_message and "terminate" in last_message and last_message["terminate"]=="yes":
                    self.browser_manager.notify_user(last_message, "answer") # type: ignore

            self.browser_manager.notify_user(f"Task Completed ({elapsed_time}s).", "info") # type: ignore
            self.browser_manager.command_completed(command, elapsed_time) # type: ignore
            self.is_running = False

    def save_planner_chat_messages(self):
        """
    ...
    def wait_for_exit(self):
        ...
        """
        self.shutdown_event.wait()  # Wait until the shutdown event is set
