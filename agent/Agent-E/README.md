...
1. On Linux/macOS, run the following command:
   ...
   ```
2. On Windows, run the same command but without ```--reload``` (Python still has different implementations across OSes, removing --reload helping finding a workaround, see this [answer on  StackOverflow](https://stackoverflow.com/a/78795990)):
   ```cmd
