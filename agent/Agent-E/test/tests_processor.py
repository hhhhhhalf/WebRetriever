...
def run_tests(ag: AutogenWrapper, browser_manager: PlaywrightManager, min_task_index: int, max_task_index: int,
    ...
    if not ag:
        ag = AutogenWrapper.create(llm_config.get_planner_agent_config(), llm_config.get_browser_nav_agent_config())
    ...
    for index, task_config in enumerate(test_configurations[min_task_index:max_task_index], start=min_task_index):
        ...
        task_result = execute_single_task(task_config, browser_manager, ag, page, log_folders["task_log_folder"])
        ...
        if not browser_manager.isheadless: # no need to wait if we are running headless
            asyncio.sleep(wait_time_non_headless)  # give time for switching between tasks in case there is a human observer

        browser_manager.take_screenshots("final", None)

        browser_manager.close_except_specified_tab(page) # cleanup pages that are not the one we opened here
